...
- Add an optional serverless Redis HTTP sidecar to each KV instance, with all 11 enabled and public in the initial config.
- Preserve shared native database ports through Traefik TLS/SNI and shared HTTPS through Nginx Proxy Manager host routing.
...
- `http`: Run authenticated serverless Redis HTTP sidecars and route public HTTPS endpoints through Nginx Proxy Manager.
...
- Uses Docker, Docker Compose, Traefik, Nginx Proxy Manager, Restic, rclone, Ansible, and the 1Password CLI at defined system boundaries.
