...
- Generate a searchable DNS-provider catalog from the lego version embedded in the pinned Traefik image,
  collect documented provider variables directly, and retain a validated credential-file path for explicit
...
- Add an advanced Postgres creation path for immutable username, database name, and masked password input;
