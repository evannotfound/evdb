...
`evdb status` SHALL report every configured database's running state, engine health, deployment drift, latest confirmed remote backup, and latest successful full backup verification in one concise table.
...
Status SHALL compare the active release, locked images, generated configuration, shared network, Traefik, and every expected primary and sidecar container's image, health, and service-contract label and SHALL distinguish pending desired changes from unplanned live drift.
